...
static _MP_Memory* extend_memory_list(ld_mempool* mp, mem_size_t new_mempool_sz) {
...
static _MP_Memory* find_memory_list(ld_mempool* mp, void* p) {
...
static int merge_free_chunk(ld_mempool* mp, _MP_Memory* mm, _MP_Chunk* c) {
    _MP_Chunk *p0 = c, *p1 = c;
    while (p0->is_free) {
        p1 = p0;
...
ld_mempool* mempool_init(mem_size_t max_mempool_size, mem_size_t mempool_size) {
    ...
    ld_mempool* mp = (ld_mempool*) malloc(sizeof(ld_mempool));
    if (!mp) return NULL;
